...
dates$mon <- seq(0,length(seq(as.Date("2019-01-01"), as.Date("2021-12-01"), by = 'month'))-1)
...
data_cols <- c("obs", names(hiik)[grepl("intensity", names(hiik))])
hiik <- merge(hiik[, .(country = unlist(strsplit(countries, ", "))), by = .(obs, type)], hiik[, ..data_cols])

hiik <- merge(hiik, isos[,c("iso3", "countryname_hiik")], by.x = "country", by.y = "countryname_hiik", all.x = T)

hiik <- melt(hiik, id.vars = c("obs", "iso3", "country", "type"))
...
acaps_conflict <- merge(acaps_conflict, hiik[year == 2019], by = c("iso3", "type", "obs"), suffixes = c("_18", "_19"))
...
#Displacement
displacement <- data.table(read_excel("S:/Projects/GHA/Phase IV/Projects/P0489 GHA Programme 2022/GHA Report 2022/Project Content/1. Research and analysis/3. Datasets/Displacement/Displacement Data.xlsx", sheet = "Displaced population by country", range = "A3:H204"))
displacement <- melt(displacement[, -"Country of asylum"], id.vars = "ISO")

displacement <- merge(displacement, pop[area == "total"], by.x = c("ISO", "variable"), by.y = c("ISO3", "year"), all.x = T)
displacement[, share_displaced := value/population]

acaps_displacement <- acaps_flags[Displacement == 1 & year == 2019]
acaps_displacement <- merge(acaps_displacement[, year := 2018], displacement, by.x = c("iso3", "year"), by.y = c("ISO", "variable"))
acaps_displacement <- merge(acaps_displacement, displacement[variable == 2019], by.x = c("iso3"), by.y = c("ISO"), suffixes = c("_18", "_19"))
acaps_displacement <- acaps_displacement[share_displaced_18 >= 0.9*share_displaced_19, .(iso3, year)]

min_displacement_share <- displacement[paste0(ISO, variable) %in% acaps_flags[acaps_flags[Displacement == 1, .I[which.min(year)], by = iso3]$V1, paste0(iso3, year)], min(share_displaced, na.rm = T)]

displacement[share_displaced >= min_displacement_share & paste0(ISO, variable) %in% lccs[, paste0(iso3, year)] & paste0(ISO, variable) %in% vulns[, paste0(iso3, year)], displacement_marker := T]

displacement <- unique(rbind(unique(displacement[displacement_marker == TRUE, .(iso3 = ISO, year = variable)]), acaps_flags[Displacement == 1, .(iso3, year)], acaps_displacement))
...
fwrite(all_crises, "crisis_types.csv")
